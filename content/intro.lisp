...
(defnode (intro . why) +qua-hub-section+
  ...
  (:child
   ...
   pervasive in JavaScript.  At some point I decided I simply wasn't
   going to write any more algorithms in the callback-style model
   required by most JavaScript APIs.  Life's too short for programming
   without a control stack, and the alternatives offered, like
   promises or the ``generator hack'' are just as bad.  I decided I
   wanted a very lightweight language, directly and seamlessly
   embedded into JavaScript, with some form of advanced control stack
   manipulation facility, like coroutines or continuations, so that I
   could invert the broken async model and let my algorithms do all
   the things that they should be able to do: block on IO, sleep,
   etc.")
   (paragraph "I studied various concurrency and control flow
   manipulation models and eventually settled on delimited
   continuations as the main control flow model of Qua.  Delimited
   continuations have emerged as what most people would consider the
   right way to add continuations to a language, and are essentially a
   more elegant formulation of the same functionality as coroutines.
   If you understand e.g. Lua coroutines, you already understand
   delimited continuations, too, and just have to learn the new
   interface, which is beautiful and quite mind-blowing.  With
   delimited continuations, Qua programmers can at any point get a
   copy of the control stack as a datum that can be stored in a data
   structure etc.  This means, the current Qua computation can be
   ``parked'' when an asynchronous JS API call is about to be done,
   and later be restarted when Qua gets called back.  So all
   asynchronous APIs can be inverted to a synchronous style from the
   point of view of the Qua programmer.")))
...
(defnode (intro . other) +qua-hub-section+
  ...
  (:child
   ...
   variant of Lisp, called Kernel, which reintroduced an often
   misapprehended and disparaged language construct called the
   ``fexpr''.  After some investigation and gasps I realized that
   fexprs are the perfect fundament on which to build an ultralight
   language.  A fexpr is like a function but instead of receiving the
   evaluated values of its arguments (as in ``call-by-value''), it
   receives their unevaluated source expressions (``call-by-text'')
   and the first-class lexical environment in which the fexpr is
   called.  It turns out that this simple setup offers about the same
   metaprogramming power as Scheme's most advanced hygienic macro
   systems, but with none of the headaches or flashbacks: hygiene is
   simply achieved with the lexical scope already built into the
   language.  This means that once you put the very simple, in fact
   trivial, concepts of fexprs and first-class environments into a
   language, you have sufficient tools to bootstrap the rest of the
   language from there, because fexprs effectively combine the power
   of functions and hygienic macros (and more) into a single
   construct.  And to top it off, fexprs are a lot of fun, too, so it
   became a no-brainer to use them as the basis of Qua.")
